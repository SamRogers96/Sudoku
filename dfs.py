...
#DFS uses a stack to search the tree thus always viewing the tree column by column

def DFS(problem):
    # Generate the initial (root) node
    node = Node(problem.initial)

    # For efficiency, we check if the node is a goal state BEFORE putting on the stack
